    ...
    with open("Ovinger/Oving_4/responstider_ms.txt", "r", encoding="UTF-8") as responstid:
        for linje in responstid:
            ...
            if " ms" not in linje_strip:
                skriv_logg("Feil eller manglende ms", linje)
                feil_linje += 1
            else:
                ...
                tid = float(linje_strip.replace(" ms", "")) # For å unngå at den legger til linjeskift i listen, må også gjøre om til float for å kunne regne på
                if tid > 20:
                    for_hoy_maaling += 1
                elif tid < 15:
                    ok_maaling += 1
